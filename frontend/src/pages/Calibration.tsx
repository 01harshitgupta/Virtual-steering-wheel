    ...
    <div className="flex-1 p-6 md:p-8 flex flex-col gap-6 select-none max-w-7xl mx-auto w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 flex flex-col gap-4">
          ...
          <div className="glass-card p-4 rounded-xl border border-slate-800/60 flex gap-3 text-xs leading-relaxed text-slate-400">
            <HelpCircle className="w-5 h-5 text-cyan-400 shrink-0" />
            <div>
              <span className="font-semibold text-slate-300 block mb-1">Calibration Pro-Tips:</span>
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          ...
          <div className="glass-card p-5 rounded-2xl border border-slate-800/80 shadow-xl flex flex-col gap-4">
            <h2 className="text-sm font-semibold uppercase tracking-wider text-slate-400 flex items-center gap-2">
              <Compass className="w-4 h-4 text-cyan-400" />
              Progress Timeline
            </h2>

            <div className="flex flex-col gap-3.5 relative pl-4 border-l border-slate-800/80 ml-2 py-1">
              {steps.map((step, idx) => {
                  ...
                  <div key={idx} className="relative flex flex-col gap-0.5">
                    ...
                    <div
                      className={`absolute left-[-21px] top-1.5 w-3.5 h-3.5 rounded-full border-2 transition-all ${
                        isCurrent
                          ? "bg-cyan-400 border-cyan-400 shadow-md shadow-cyan-400/40 scale-110"
                          : isPassed
                    ...
                    <span
                      className={`text-xs font-semibold ${
                        isCurrent ? "text-cyan-400" : isPassed ? "text-slate-300" : "text-slate-500"
                      }`}
                    >
                      {step.title}
                    </span>
          ...
          <div className="glass-card p-6 rounded-2xl border border-slate-800/80 shadow-xl min-h-[220px] flex flex-col justify-between">
            {/* Step Content */}
            {calibrationStep === 0 && (
              <div className="flex flex-col gap-3">
                <h3 className="text-lg font-bold text-white">1. Connect & Position</h3>
            ...
            {calibrationStep === 1 && (
              <div className="flex flex-col gap-3">
                <h3 className="text-lg font-bold text-white">2. Centering Alignment</h3>
                <p className="text-sm text-slate-400 leading-relaxed">
            ...
            {calibrationStep === 2 && (
              <div className="flex flex-col gap-3">
                <h3 className="text-lg font-bold text-white">3. Maximum Left Lock</h3>
                <p className="text-sm text-slate-400 leading-relaxed">
            ...
            {calibrationStep === 3 && (
              <div className="flex flex-col gap-3">
                <h3 className="text-lg font-bold text-white">4. Maximum Right Lock</h3>
                <p className="text-sm text-slate-400 leading-relaxed">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-3 gap-2 bg-slate-950/60 p-3 rounded-xl border border-slate-900 font-mono text-center">
                  <div className="flex flex-col">
                    <span className="text-[10px] text-slate-500">LEFT LOCK</span>
                    <span className="text-xs font-bold text-rose-400">{leftBoundVal}°</span>
                  </div>
                  <div className="flex flex-col border-x border-slate-900">
                    <span className="text-[10px] text-slate-500">CENTER</span>
                    <span className="text-xs font-bold text-cyan-400">{centerOffsetVal}°</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[10px] text-slate-500">RIGHT LOCK</span>
                    <span className="text-xs font-bold text-emerald-400">{rightBoundVal}°</span>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="flex items-center justify-between gap-3 mt-6 pt-4 border-t border-slate-900/60">
              ...
              {calibrationStep < 4 ? (
                <button
                  ...
                  className="ml-auto px-5 py-2.5 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold text-xs uppercase tracking-wider flex items-center gap-1.5 shadow-lg shadow-cyan-500/10 disabled:opacity-50 disabled:pointer-events-none transition-all duration-200"
                >
                  <span>
                    {calibrationStep === 0
                      ? "Get Started"
                ...
                <button
                  onClick={handleReset}
                  className="ml-auto px-5 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-slate-200 border border-slate-800 hover:border-slate-700 font-bold text-xs uppercase tracking-wider flex items-center gap-1.5 transition-all"
                >
